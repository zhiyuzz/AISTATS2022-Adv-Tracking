import numpy as np
from Algorithms.Subroutine_ball_shifted import SubroutineBallShifted
from Algorithms.Subroutine_1d import SubroutineOneD


class MetaNormBallShifted:
    ...
    def get_prediction(self):
        ...
        for k in range(self.K_plus_one):
            if self.t % (2 ** k) == 0:
                shift = np.zeros(self.d)    # the default shift is 0
                if k in self.ALGs_ball:
                    shift = self.ALGs_ball[k].get_prediction()
                self.ALGs_ball[k] = SubroutineBallShifted(self.lam, self.eps_0 * 2 ** k, self.G, self.d, self.R, shift)
